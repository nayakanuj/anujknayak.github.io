...
![private classical comm table](/images/private_classical_comm_table.png)
...
![qcomm toy eg animated](/images/qcomm_animated.gif.webp)
...
Suppose Alice has an entangled state $|\psi\rangle_{RA}$, here the subscripts denote two quantum systems R and A (Reference and Alice, respectively). Here assume both R and A are Alice's systems, have the same dimensions, and are entangled with each other. Consider an example entangled state $\left( a |00\rangle + b |11\rangle \right)$. The entanglement between R and A are indicated by the squiggly line between first and second qubit (counting from top to bottom) in the animation above. With a goal of creating shared entanglement with Bob, Alice decides to send qubit A to Bob. First, she encodes the qubit A as: $|0\rangle \rightarrow \frac{1}{\sqrt{2}} \left( |00\rangle + |01\rangle \right)$ and $|1\rangle \rightarrow \frac{1}{\sqrt{2}} \left( |10\rangle + |11\rangle \right)$. This encoding is chosen because as we will see later, the environment entangles with the second qubit (similar to the private communication in the classical case). Instead of randomly choosing one code word as in the classical case, we construct a uniform superposition over all codewords in the "randomization" set (across columns indexed by k in the previous subsection). Consequently, the state after encoding becomes:

$\left( a |00\rangle + b |11\rangle \right) \rightarrow \frac{a}{\sqrt{2}} |0\rangle \otimes \left( |00\rangle + |01\rangle \right) + \frac{b}{\sqrt{2}} |0\rangle \otimes \left( |10\rangle + |11\rangle \right)$.

Now, suppose the environment interacts (entangles) with the encoded qubits in the following manner:

$\frac{a}{\sqrt{2}} |0\rangle \otimes \left( |00\rangle + |01\rangle \right) + \frac{b}{\sqrt{2}} |0\rangle \otimes \left( |10\rangle + |11\rangle \right) \\ \rightarrow \frac{a}{\sqrt{2}} |0\rangle \otimes \left( |00e_0\rangle + |01e_1\rangle \right) + \frac{b}{\sqrt{2}} |0\rangle \otimes \left( |10e_0\rangle - |11e_1\rangle \right)$
...
$\left( a |00\rangle + b |11\rangle \right) \otimes \frac{1}{\sqrt{2}} \left( |0e_0\rangle + |1e_1\rangle \right)$.
...
In quantum information theory, instead of typical sets as in the classical case there is a notion of typical subspaces. This arises due to the superposition of quantum states. For example, suppose 0…000 and 1…111 belong to the typical set, there is no notion of superpositions in the classical case. On the other hand, in the quantum setting the uncountable set of all superpositions of $|0...000\rangle$ and $|1...111\rangle$ are also typical, which gives rise to the notion of a typical subspace (also referred to as coherent version or informally "coherification" of the classical code). Fortunately, in most cases, classical typicality arguments carry over directly to the quantum case, which helps in determining the dimensionality of the typical subspace and consequently, the rate of information transmission over quantum channels.

![qcomm gen](/images/qcomm_gen-1.png)
...
Consider a density matrix $\rho_A = \sum\limits_x p_X(x) |\psi_x\rangle \langle \psi_x|_A$. Let $\{x^n(m, k)\}_{m \in \mathcal{M}, k \in \mathcal{K}}$ be a random codebook constructed i.i.d. using the distribution $p_X(x)$. From private capacity conditions, for $n$ uses of the quantum channel, if $|\mathcal{M}| \approx 2^{n[I(X ; B)-I(X ; E)]}$ and $|\mathcal{K}| \approx 2^{n I(X ; E)}$, then the following conditions must hold.
...
Suppose Alice creates an entangled quantum state $|\varphi\rangle_{R A} \equiv \sum\limits_m a_m |m\rangle_R |m\rangle_A$ in her lab (Alice has access to both the systems R and A). Suppose Alice wants to transmit her subsystem A to Bob. Then, the encoder $\mathcal{E}$ maps each $|m\rangle_A$ in the superposition coherently to for a superposition of quantum codewords as follows:

$\mathcal{E}: |m\rangle_A \rightarrow |\phi_{m}\rangle_{A^{n}} \equiv \frac{1}{\sqrt{|\mathcal{K}|}} \sum_{k \in \mathcal{K}} |\psi^{x^{n}(m, k)}\rangle_{A^{n}}$

where $A^n$ denotes $n^{th}$ extension of Alice's subsytem A. Here $x^n$ is chosen from the typical set under the distribution $p_X(x)$, and $|\psi^{x^{n}(m, k)}\rangle = |\psi^{x_0}\psi^{x_1} . . .\psi^{x_n}\rangle$. Moreover, note that instead of randomization over $k$, quantum codewords are uniform superposition over $k$, which can be interpreted as a quantum version of choosing codewords uniformly at random among $\mathcal{K}$ codewords. After encoding, the quantum state (of the joint system $R, A^n$) becomes:

$\sum_{m} \alpha_{m}|m\rangle_{R} |\phi_{m}\rangle_{A^{n}}$
...
$\sum_{m} \alpha_{m}|m\rangle_{R} |\phi_{m}\rangle_{B^nE^n}$
...
Bob creates a superposition $\sum_{m, k} |m\rangle |k\rangle$ in subsystems $B_1 B_2$ and then performs a coherent measurement on $B^n B_1 B_2$ such that each rotated codeword $|\phi_{m}\rangle_{B^nE^n}$ (rotated by the channel) is almost uniquely entangled with $| m \rangle | k \rangle$ using the subspace projector $\Lambda_{B^{n}}^{m, k}$, which yields the following state:

$\sum_{m} \sum_{k} \frac{1}{\sqrt{|\mathcal{K}|}} \alpha_{m}|m\rangle_{R} |\psi^{x^n(m,k)}\rangle_{B^nE^n} |m\rangle_{B_1} |k\rangle_{B_2}$

In the Figure, this operation is shown by block with a meter symbol on $B^n$ connected to xor's on $B_1$ and $B_2$.

Finally, there exists a controlled isometry $\sum\limits_{m}|m\rangle \langle m|_{B{1}} \otimes U_{B^{n} B_{2} \rightarrow B_{3}}^{m}$ such that signal and noise are decoupled. That the entanglement of the environment with Bob's subsystems is restricted only to subsystem $B_3$, and the superposition of codewords is decoded (mapped) to superposition of messages in subsystem $B_1$.

$\left(\sum_{m \in \mathcal{M}} \alpha_{m}|m\rangle_{R} |m\rangle_{B_{1}}\right) \otimes|\theta\rangle_{B_{3}E^{n}}$
...
$\log |A| = I(A\rangle B)_\Phi \leq I(A\rangle B)_{\omega} + 2 \epsilon \log |A| + h_2(\epsilon)$

$\leq I(A\rangle B^n)_{\omega*} + 2 \epsilon \log |A| + h_2(\epsilon) \leq Q(\mathcal{N}^{\otimes n}) + 2 \epsilon \log |A| + h_2(\epsilon)$
